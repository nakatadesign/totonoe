...
> この RUNBOOK は totonoe v3 系ランタイムを前提としています。
...
```bash
.totonoe/bin/status.sh --job-name sample-feature --provider-state
```

急いで止めたい場合は、Claude Code 側で `totonoe stop` と伝えるか、手元で次を実行する。

```bash
.totonoe/bin/pause_job.sh --job-name sample-feature --reason "user requested stop"
```
...
.totonoe/bin/record_claude_round.sh \
  --job-name sample-feature \
  --summary-file .totonoe/runtime/sample-feature/summary.md \
...
```bash
.totonoe/bin/run_reviewer.sh --job-name sample-feature
.totonoe/bin/run_judge.sh --job-name sample-feature
```
...
> **注意**: API キーは `.env` に書き、`.totonoe/config.json` には入れないこと。`config.json` にはモード設定など公開可能な情報のみを置く。
> `gemini-2.5-pro` は project / tier によって free tier で使えない場合があるため、既定モデルは fallback / shadow 用の軽量な `gemini-2.5-flash-lite` としている。

## 5. Manager が確認して決定する

```bash
.totonoe/bin/apply_manager_decision.sh \
  --job-name sample-feature \
  --record-spot-check
...
```bash
# 全体サマリー
.totonoe/bin/query_knowledge.sh --type summary

# 未解決の過去指摘（severity 別）
.totonoe/bin/query_knowledge.sh --type findings --severity critical --limit 10

# 過去の Judge 判定傾向
.totonoe/bin/query_knowledge.sh --type verdicts --engineer-type security --limit 5
```
...
```bash
.totonoe/bin/query_knowledge.sh --type lessons --limit 5
```
...
```bash
.totonoe/bin/reset_provider.sh --job-name sample-feature
```
...
```bash
jq 'select(.type == "ai_exec")' .totonoe/runtime/sample-feature/events.jsonl
```
