...
`totonoe` は複数の定型スクリプトを連続実行する構成のため、`claude --permission-mode acceptEdits` のような控えめなモードでも確認ダイアログは残りやすく、長いループでは途中停止が発生しがちです。
一方で `--dangerously-skip-permissions` は権限確認を省略できるため、review / judge / manager の流れを中断せずに回しやすくなります。

ただし、これは安全な隔離環境で使うことを前提とします。
普段使いの開発マシンや、本番 credential・個人ファイルにアクセスできる環境では推奨しません。
